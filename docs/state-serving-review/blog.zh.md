...
![图 5：代表性系统公开给出的上下文窗口信号](assets/fig05_context_windows.png)

*图 5. 长上下文的公开信号仍然重要，但它只是服务系统的一部分接口。数据来自模型卡与官方文档。*

![图 6：从长窗口到长时服务的产品信号](assets/fig06_service_horizon.png)
...
![图 2：长上下文下 KV Cache 仍会成为瓶颈](assets/fig02_kv_cache_curve.png)
...
### 2.3 把 DeepSeek-V4 的 1M cache 算一遍

用 DeepSeek-V4-Pro 做一个数量级算例，可以把前面的抽象公式变得更具体。

公开 config 给出的几个关键量是：`num_hidden_layers = 61`，`num_key_value_heads = 1`，`head_dim = 512`，`sliding_window = 128`，压缩率序列里有 30 个 `m = 4` 的 CSA 层、31 个 `m = 128` 的 HCA 层，另有一个结尾占位 `0` 不计入压缩 cache。

因此，DeepSeek-V4 的长度相关 cache 不应再按普通 Transformer 的 `T × L` 来算，而应写成：

`KV_DS(T) ≈ 2 × bytes × H_kv × d_head × Σ_l ceil(T / m_l)`

如果先用 BF16 cache 口径，也就是 `bytes = 2`，那么每个 compressed KV entry 需要：

`2 × 2 × 1 × 512 = 2048 bytes`

而所有层的有效压缩斜率为：

`Σ_l 1 / m_l = 30 / 4 + 31 / 128 = 7.7421875`

所以 DeepSeek-V4-Pro 的压缩 KV cache 斜率大约是：

`2048 × 7.7421875 = 15,856 bytes / token`

放到 1M token 上，就是约 `14.8 GiB`；如果按精确的 `1,048,576` token 计算，则约 `15.5 GiB`。同一组层数和 KV 头宽如果完全不压缩，即使仍然只按 MQA 的一个 KV head 算，也会到约 `122 GiB`。这解释了为什么论文会把百万上下文的关键收益写成 KV cache 只剩 DeepSeek-V3.2 的一小部分。

还要注意，SWA 的 state cache 在这个尺度下不是主项。按 `61 × 128` 个最近窗口 KV entry 粗算，它只有十几 MiB；即使加上每层未凑满 compression block 的 tail state，也远小于长度相关的 compressed KV。

真正大的常驻项仍然是权重。DeepSeek-V4-Pro 总参数是 1.6T，激活参数是 49B；激活参数主要决定每 token 的 MoE 计算量，不等于只需要加载 49B 权重。推理集群通常仍要把专家权重分片常驻。若把大部分 expert weight 按 FP4、其他权重按 FP8 估算，权重常驻显存大约在 `0.85-0.95 TB` 量级，再加上每条 1M 序列约 `15 GiB` 的 BF16 cache、运行时 workspace、通信 buffer 和框架开销。因此，单条百万上下文请求的“能跑起来”不是 15 GiB 问题，而更接近一个接近 TB 级权重常驻系统上的 per-sequence cache 增量问题。

这也给 state-based 架构一个直接参照：如果某个 Gated DeltaNet / Kimi Linear 式系统把运行时记忆做成固定 recurrent state，那么它不必和 1.6T 参数或 39B/49B 这类 activated-parameter 口径直接等大，而要和 DeepSeek-V4 的 per-sequence cache 斜率比较。

一个矩阵状态的粗略公式可以写成：

`State bytes ≈ L_state × H_state × d_state × d_state × bytes × n_state_slots`

这里 `n_state_slots` 可以理解成 KDA/Gated DeltaNet 里多组 fast-weight、门控通道或 expanded state lane 的常数因子。比如，`L_state = 61`、`H_state = 64`、`d_state = 512`、BF16、`n_state_slots = 4` 时，固定 state 约为 `7.8 GiB`；如果把 head 数或 slot 数再翻倍，就会来到 `16 GiB` 量级。

这时交叉点很直观：

| 固定 recurrent state | 何时大于 DeepSeek-V4 压缩 KV | 何时小于 DeepSeek-V4 压缩 KV |
|---:|---:|---:|
| 8 GiB | 约 54 万 token 以内 | 超过约 54 万 token |
| 16 GiB | 约 108 万 token 以内 | 超过约 108 万 token |
| 32 GiB | 约 217 万 token 以内 | 超过约 217 万 token |

![图 9：DeepSeek-V4 cache 增长与固定 recurrent state 的交叉点](assets/fig09_deepseek_state_break_even.png)

*图 9. 蓝线是 DeepSeek-V4-Pro 按公开 config 估算的 BF16 压缩 KV cache；水平线是几种固定 recurrent state 预算。含义不是“state 一定更省”，而是 state size 一旦做大，也会在中短上下文区间超过压缩 KV。*

这张图给出的结论有点反直觉：state-based 架构不是天然显存更低，它只是把长度相关成本变成了一个可选择的固定预算。如果为了追求 DeepSeek-V4 级别能力，把 state 做到 32 GiB，那么在 1M token 以内，它反而比 DeepSeek-V4 的压缩 KV 更重；只有当服务历史继续增长到 2M token 以上，它才开始表现出固定状态的优势。

更复杂的是，Kimi Linear 这种 3:1 KDA-to-MLA hybrid 并不是 pure recurrent。若放大到 DeepSeek-V4 这种层数和 MoE 规模，同时保留约四分之一 global MLA 层，那么仍会有一条随 `T` 增长的 token-addressable cache。假设保留 15 个 MLA 层、每层 latent cache 约 `512 + 64` 维、BF16，那么仅 MLA cache 在 1M token 下就约 `16 GiB`。如果再叠加一个 16 GiB recurrent state，总 per-sequence memory 会到 30 GiB 以上。

所以，对 Kimi Linear / Gated DeltaNet 这类路线来说，关键设计不是“state 越大越好”，而是要把 state、少量 attention 兜底和外部 memory 放在同一个预算里共同优化。

### 2.4 Serving 文献中的一个关键信号：状态正在被拆分
...
![图 1：从长上下文到 state-centric serving 的路线图](assets/fig01_architecture_landscape.png)
...
### 5.5 Prefix cache 命中率决定 state 应该做多大

把这个问题放进 serving 经济学里，结论会更清楚。

DeepSeek-V4 这类压缩 token-cache 路线的强项，是它仍然保留了可复用的前缀对象。只要多个请求共享同一份长文档、同一个代码库、同一个系统 prompt 或同一段 agent 轨迹，prefix cache 命中后，昂贵的 prefill 可以被摊薄。它的弱项也很直接：只要活跃历史继续增长，per-sequence KV memory 和 cache transport 压力仍会增长。

Kimi Linear / Gated DeltaNet 这类 state 路线的强项，是把大量历史压进固定状态，状态对象更小、更容易搬运，也更适合长时间连续执行。它的弱项是：如果 state 太小，模型会更依赖 retrieval 和 tool verification；如果 state 太大，它会在中短上下文区间直接超过 DeepSeek-V4 这种压缩 KV 的显存成本；如果还保留少量 MLA / attention 层，则 token cache 并没有完全消失。

因此，一个更接近真实部署的目标函数应该写成：

`Cost = weight_rent + live_memory_rent + prefill_miss_cost + cache_io_cost + retrieval_cost + forgetting_loss`

其中 DeepSeek-V4 路线大致是：

`live_memory_rent ∝ T × compressed_KV_slope`

而 state-first 路线大致是：

`live_memory_rent ∝ S_state + T × residual_attention_slope`

这里的 `S_state` 才是最关键的旋钮。它不应该按模型参数量机械放大，而应该按业务工作负载选择。

如果请求高度共享前缀，例如企业知识库问答、同一个 repo 上的大量代码请求、同一个长文档上的多轮问答，那么 DeepSeek-V4 式 cache 经济学会很强。此时 state-first 模型不应该把 recurrent state 做得过大，因为大 state 会占用常驻显存，却未必比 prefix cache 带来更高边际收益。

如果请求前缀高度个性化，例如长时 coding agent、个人工作流 agent、跨小时 browser automation，prefix cache 命中率会下降，历史也会不断产生新的中间状态。这时 fixed state 的价值会上升，因为它把不断增长的 token history 变成了可控的 live state 预算。

所以，对 DeepSeek-V4 级别的 Kimi Linear / Gated DeltaNet，我会把设计策略写成一句话：

> **先把 hot recurrent state 控制在 8-16 GiB 量级，再用少量低秩 attention 层、外部 memory 和 tool verification 补精确召回；只有当目标工作负载明确需要数百万 token 以上的低命中长服务，才考虑把 state 推到 32 GiB 或更高。**

这个结论和图 9 是一致的。16 GiB state 在 1M token 附近已经接近 DeepSeek-V4 的压缩 KV 成本；32 GiB state 要到 2M token 以上才开始明显体现固定状态的显存优势。换句话说，state size 的经济学不是“越大越接近终局”，而是“越大越像提前购买一段未来服务长度”。
...
![图 3：面向 state-centric serving 的四层记忆栈](assets/fig03_memory_stack.png)
...
![图 7：长时 Agent 的状态服务闭环](assets/fig07_state_serving_loop.png)
...
![图 8：面向 state-centric serving 的评估地图](assets/fig08_evaluation_map.png)
...
![图 4：从状态复用到状态资产化的最小生命周期](assets/fig04_state_market.png)
...
7. 图 9：DeepSeek-V4 与固定 state 的交叉点
   用来把 state-centric serving 的显存权衡落到具体数量级。
