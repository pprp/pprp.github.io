...
**上下文**指的是从大型语言模型（LLM）进行采样时，被放入模型视野中的 token 集合。这里的**工程**问题，是在 LLM 固有约束下最大化这些 token 的效用，使模型持续朝预期目标行动。有效驾驭 LLM，往往需要_以上下文的视角思考_：也就是考量模型在任意时刻可见的整体状态，以及这种状态可能诱发的行为。
