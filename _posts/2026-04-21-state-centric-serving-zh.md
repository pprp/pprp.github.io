---
layout: post
title: '从超长上下文到终身服务：为什么开源模型最终会走向 State-Centric Serving'
date: 2026-04-21 00:00:00 +0800
categories: tech
---

_一篇把架构分化、长时 Agent、外部记忆与状态资产化放在同一张系统图上的研究型博客。_
...
| Kimi Linear（Moonshot）                | 技术报告把 KDA 与周期性全局注意力/MLA 的 3:1 Hybrid 结构、KV cache 降低和 1M context decode 吞吐放在同一条论证链里            | Hybrid linear attention 已经不只是复杂度符号，而是被直接包装成 serving 收益 |
| Qwen3.5-397B-A17B（Qwen Team）         | 模型卡公开 `15 * (3 * (Gated DeltaNet -> MoE) -> 1 * (Gated Attention -> MoE))` 的隐藏层布局，原生 262K、可扩展到约 1M tokens | 线性状态层与少数 attention 层的分工被直接写进公开模型结构                   |
| Ring-2.5-1T（inclusionAI / Ling Team） | 模型卡称其基于 hybrid linear attention，采用 1:7 MLA + Lightning Linear Attention，并把 32K+ 长序列吞吐作为核心卖点           | 混合线性注意力开始进入 trillion-scale thinking / agent 模型叙事             |
...
![图 5：代表性系统公开给出的上下文窗口信号](../../images/state-serving/fig05_context_windows_zh.png)

_图 5. 长上下文的公开信号仍然重要，但它只是服务系统的一部分接口。数据来自模型卡与官方文档。_

![图 6：从长窗口到长时服务的产品信号](../../images/state-serving/fig06_service_horizon_zh.png)

_图 6. 更值得关注的变化，是长任务被显式产品化：规划、连续执行、工具使用与长时交付正在进入主流叙述。_
...
![图 2：长上下文下 KV Cache 仍会成为瓶颈](../../images/state-serving/fig02_kv_cache_curve_zh.png)

_图 2. 即便只保留少量 attention 层，长度相关的 cache 压力依然会随着上下文增长持续放大。_
...
![图 1：从长上下文到 state-centric serving 的路线图](../../images/state-serving/fig01_architecture_landscape_zh.png)

_图 1. 这张图有意把“已观察到的工程响应”和“本文的前瞻性判断”分开，以避免把推演写成既成事实。_
...
| Hybrid             | 用 bounded recurrent / linear state 替代大部分 attention 层 | Qwen3.5、Kimi Linear、Ring-2.5 / Ling 2.5、OLMo Hybrid | 把大部分长度相关缓存改成长度无关状态 | 仍需 attention 层承担精确检索与兜底 |
| Sparse / MLA / SWA | 保留 token-addressable memory，但降低覆盖成本               | DeepSeek-V2/V3.x、GLM 系列、MiMo-V2-Flash、滑窗系统    | 在 recall 与成本之间取得较好平衡     | 记忆仍然主要以 token 形式被维护     |
...
#### Qwen3.5：公开地把 attention 留在少数层

Qwen3.5-397B-A17B 的模型卡之所以重要，不只是因为它给出了长上下文数字，更因为它直接公开了一个对本文判断很关键的结构事实：

- 语言模型部分共有 60 层，隐藏层布局写成 `15 * (3 * (Gated DeltaNet -> MoE) -> 1 * (Gated Attention -> MoE))`。
- 换句话说，大约 3/4 的层已经转向 Gated DeltaNet 一类的线性状态更新。
- 剩余 1/4 的 Gated Attention 层继续承担更接近 token-addressable / global attention 的功能。
- 原生上下文能力达到 262,144，并可扩展到约 1,010,000 tokens。
...
而 Qwen3.5 这类结构的选择，等于在模型内部做了一次显式分工：少数层负责更接近精确召回的功能，多数层负责状态更新和压缩。

这不是一个“细节参数”，而是模型系统对长上下文成本压力的结构性回应。

#### Kimi Linear：把 Hybrid 的 serving 收益写得更直接

Moonshot / Kimi Team 的 `Kimi Linear` 技术报告给出的信息同样关键。

它明确写到：

- 使用 Kimi Delta Attention（KDA）作为线性注意力模块，并周期性插入 MLA / global attention 层。
- 采用 3:1 的 KDA-to-global-attention 层间混合比例。
- 在长序列生成中，KV cache 可降低到最高约 75%。
- 在 1M context 的 decode 场景中，吞吐提升被报告为最高约 6 倍量级。
...
#### Ring-2.5 / Ling 2.5：Hybrid linear 进入 trillion-scale thinking model

Ring-2.5-1T 的公开模型卡让这个趋势更明显。

它把自己定位成基于 hybrid linear attention architecture 的开源 trillion-parameter thinking model，并把长时推理、长程任务执行和 agentic RL 放在同一张叙事里。

更具体地说，Ling 2.5 架构把原来的 GQA 路线增量改造成 1:7 的 MLA + Lightning Linear Attention：

- 多数层转向 Lightning Linear Attention，以降低长序列生成中的状态和访存压力。
- 少数层保留 MLA，用来压缩 KV cache，同时保留更强的全局信息通道。
- 模型卡明确把这种高比例线性注意力与 32K+ 序列下的吞吐提升绑定起来。

这里有一个重要细节：Ring-2.5 / Ling 2.5 的“重型侧”是 MLA，而不是朴素 full attention。

但这并不削弱它对本文的意义。

它说明另一家开源大模型团队也在做类似结构分工：把大多数序列混合交给更便宜的线性状态层，把少数更昂贵的层保留下来承担全局召回、压缩 KV 和复杂重组。

如果说 Qwen3.5 和 Kimi Linear 证明 3:1 hybrid linear 已经成为强模型路线，那么 Ring-2.5 进一步说明，这条路线正在被推到 thinking model、long-horizon agent 和 trillion-scale MoE 的叙事里。

#### MiMo-V2-Flash：相邻证据，而不是 linear/full 的同义词

MiMo-V2-Flash 也值得放进同一节，但需要谨慎措辞。

小米团队的技术报告写得很清楚：MiMo-V2-Flash 采用的是 Sliding Window Attention 与 Global Attention 交错的 Hybrid Attention，窗口大小为 128，并采用 5:1 的混合比例。

所以，严格说它不是 Qwen3.5 / Kimi Linear / Ring-2.5 这一类“线性注意力 + 全局注意力”的同构案例。

但它仍然是一个重要旁证。

因为它采取的是同一种系统分解思路：

- 大多数层或大多数 token 交互走更便宜、更局部、更可控的路径；
- 少数全局层保留跨窗口的信息整合能力；
- 再通过 MoE、MTP、speculative decoding 等 serving 技术把长任务吞吐做上去。

这说明行业共识并不只是“linear attention 回来了”。

更准确的说法是：

> **强模型正在从“每一层都做完整全局 token mixing”转向“多数便宜状态更新 + 少数全局校正层 + serving 侧加速”的组合。**
...
![图 3：面向 state-centric serving 的四层记忆栈](../../images/state-serving/fig03_memory_stack_zh.png)

_图 3. 这张图刻意把 parameter memory、live state、external memory artifact 和 tool-ground truth 分开。因为在长时服务里，这四者的更新频率、成本、可审计性和治理方式完全不同。_
...
![图 7：长时 Agent 的状态服务闭环](../../images/state-serving/fig07_state_serving_loop_zh.png)

_图 7. 一次长时服务循环里，真正被反复操作的对象并不是纯文本历史，而是 live state、摘要、外部记忆、工具反馈和 carry-over residue。_
...
![图 8：面向 state-centric serving 的评估地图](../../images/state-serving/fig08_evaluation_map_zh.png)

_图 8. 一旦系统被当作长时服务系统来构造，它就必须被问：哪些能力在 compaction 之后还能留存？哪些错误在 tool failure 后能恢复？哪些记忆写入只是 token 浪费？_
...
![图 4：从状态复用到状态资产化的最小生命周期](../../images/state-serving/fig04_state_market_zh.png)

_图 4. 图中“Settle”与“artifact economy”都属于本文的前瞻推演，而不是已存在的行业标准。_
...
1. Qwen Team. _Qwen3.5-397B-A17B Model Card_.  
   <https://huggingface.co/Qwen/Qwen3.5-397B-A17B>

2. Kimi Team. _Kimi Linear: An Expressive, Efficient Attention Architecture_.  
   <https://arxiv.org/abs/2510.26692>

3. inclusionAI. _Ring-2.5-1T Model Card_.  
   <https://huggingface.co/inclusionAI/Ring-2.5-1T>

4. Xiaomi LLM-Core Team. _MiMo-V2-Flash Technical Report_.  
   <https://arxiv.org/abs/2601.02780>

5. DeepSeek-AI et al. _DeepSeek-V2_.  
   <https://arxiv.org/abs/2405.04434>

6. Z.AI. _Model Overview_.  
   <https://docs.z.ai/guides/overview/overview>

7. Choi, Derrick. _Run Long Horizon Tasks with Codex_.  
   <https://developers.openai.com/blog/run-long-horizon-tasks-with-codex>

### C.2 如果你想理解“为什么这是 serving 问题”

8. Qin et al. _Prefill-as-a-Service_.  
   <https://arxiv.org/abs/2604.15039>

9. MiniMax. _Why Did MiniMax M2 End Up as a Full Attention Model?_  
   <https://platform.minimax.io/docs/guides/text-m2-full-attention>

10. Ai2. _Introducing OLMo Hybrid_（newsletter entry）.  
    <https://allenai.org/newsletters/2026-03-newsletter>

### C.3 如果你想理解“为什么 state 比 prompt 更重要”

11. OpenAI. _Memories -- Codex_.  
    <https://developers.openai.com/codex/memories>

12. OpenAI. _Custom Instructions with AGENTS.md -- Codex_.  
    <https://developers.openai.com/codex/guides/agents-md>

13. OpenAI. _Compaction_.  
    <https://developers.openai.com/api/docs/guides/compaction>

14. Packer et al. _MemGPT: Towards LLMs as Operating Systems_.  
    <https://arxiv.org/abs/2310.08560>

15. Xu et al. _A-MEM: Agentic Memory for LLM Agents_.  
    <https://arxiv.org/abs/2502.12110>

16. Li et al. _MemOS: A Memory OS for AI System_.  
    <https://arxiv.org/abs/2507.03724>

17. Boschi et al. _MemX: A Local-First Long-Term Memory System for AI Assistants_.  
    <https://arxiv.org/abs/2603.16171>

### C.4 如果你想理解“该怎么评估”

18. Maharana et al. _Evaluating Very Long-Term Conversational Memory of LLM Agents_.  
    <https://arxiv.org/abs/2402.17753>

19. Wu et al. _LongMemEval_.  
    <https://arxiv.org/abs/2410.10813>

20. Lu et al. _Beyond a Million Tokens_.  
    <https://arxiv.org/abs/2510.27246>

21. Bandi et al. _MCP-Atlas_.  
    <https://arxiv.org/abs/2602.00933>

### C.5 如果你想继续往 Pure Linear 方向深入

22. Goldstein et al. _RADLADS_.  
    <https://arxiv.org/abs/2505.03005>

23. Chen et al. _Hybrid Linear Attention Done Right_.  
    <https://arxiv.org/abs/2601.22156>

24. Yang et al. _Gated Delta Networks: Improving Mamba2 with Delta Rule_.  
    <https://arxiv.org/abs/2412.06464>

25. Liu et al. _Scaling up the State Size of RNN LLMs_.  
    <https://aclanthology.org/2025.acl-long.564.pdf>

26. Pan et al. _Scaling Linear Attention with Sparse State Expansion_.  
