...
很多人以为，长上下文竞争的核心是“谁的窗口更大”：128K、256K、1M，甚至更长。

这个判断没有错，但它只看到了表层。

更深的问题不是模型一次能看多少 token，而是它能不能在数小时、数天，甚至跨会话的任务中，持续保持同一个工作状态：知道自己为什么开始，记得中途证明了什么，能恢复被压缩过的上下文，并在必要时回到工具和环境里重新验证事实。

换句话说，长上下文正在从一个模型能力指标，变成一个 serving 系统问题。

当大模型只是回答问题时，窗口长度当然重要；但当它变成 coding agent、research agent、workflow agent，并参与真实任务流时，单次窗口就只是局部投影。真正被竞争的，是服务寿命、状态恢复能力、记忆治理能力和长期成本结构。

这也是本文的核心判断：

> **长上下文的终局，不是更大的窗口，而是更长的状态连续服务。**

一旦问题从 `bigger window` 改写为 `longer service horizon`，系统设计的重心就会从“尽可能保留所有 token”转向“在可接受成本下保留足够的 live state，并在必要时通过外部记忆与工具召回高精度信息”。

在这个框架下，今天常见的 Hybrid、Sparse、MLA、SWA 等路线仍然重要。它们不是错误路线，也不是落后路线，而是当前模型能力、训练稳定性和 serving 成本之间的现实折中。

但如果把时间尺度继续拉长，真正更像终局候选的，是一种 **Linear-first + External Memory + Tool-Grounded Verification** 的栈式系统。它不是把历史全部塞进窗口，而是把状态变成系统的一等对象。

全文按三段推进。

第一段先把问题从“窗口长度”改写成“服务时长”：为什么长上下文会变成 serving 问题，为什么 KV cache、prefill/decode 带宽和上下文残留会成为系统瓶颈。

第二段进入架构分化：Full Attention、Hybrid、Sparse / MLA / SWA 与 Pure Linear 分别解决什么，又为什么都必须放进更大的 state-centric serving 语法里理解。

第三段讨论系统化后果：四层记忆栈、state-serving evaluation、state artifact 的兼容性边界，以及 token 与 state 在未来服务经济中的不同角色。

---

## 0. 先把问题说清楚：本文讨论的不是“更大的 Prompt”
...
因此，本文中的 `state-centric serving` 指的不是一个已经被行业统一命名的标准范式，而是对一组系统趋势的概括：
...
为了避免把推论伪装成事实，下面先给一个简单的证据分层。后文涉及公开模型能力、论文报告或官方工程实践时，尽量落在 A 级事实上；涉及状态资产化、结算与市场结构时，则明确保留为前瞻推演。
...
这样的比较当然重要，但它只描述了**单轮请求**的容量边界，并不能说明模型在长任务中的持续工作能力。

今天真正有价值的 Agent 系统，越来越不是“一次答对一个问题”，而是在持续规划、多轮工具调用、跨窗口状态保留、证据更新和身份连续性之间保持稳定协同。
...
窗口回答的是“这一刻能看见多少历史”；服务寿命回答的是“下一刻还能不能继续正确工作”。前者是容量问题，后者是系统问题。
...
![图 5：代表性系统公开给出的上下文窗口信号](../../images/state-serving/fig05_context_windows.png)

*图 5. 长上下文的公开信号仍然重要，但它只是服务系统的一部分接口。数据来自模型卡与官方文档。*

![图 6：从长窗口到长时服务的产品信号](../../images/state-serving/fig06_service_horizon.png)
...
因此，很多长上下文优化从根上看不是在最终“解决记忆问题”，而是在：

- 推迟爆炸点。
- 降低爆炸斜率。
- 把部分长度相关缓存改造成长度无关状态。

这也是长上下文从模型指标变成基础设施问题的原因。只要历史仍以 token cache 的形态常驻，成本就不会只体现在一次请求里，而会体现在显存占用、HBM 带宽、batching 效率、cache 迁移、跨租户隔离和恢复策略上。

### 2.2 图 2 想说明的不是曲线，而是成本结构

初稿中的图 2 仍然保留，因为它很好地说明了一个工程直觉：

![图 2：长上下文下 KV Cache 仍会成为瓶颈](../../images/state-serving/fig02_kv_cache_curve.png)
...
这也是为什么本文认为，长上下文的下一阶段不会只是更激进的 attention kernel，而更像一套 memory-serving protocol。
...
真正关键的不是“历史是否还在”，而是“历史以什么形态存在、由谁调度、如何验证、成本由谁承担”。这是 state-centric serving 与普通 long-context serving 的分界线。
...
![图 1：从长上下文到 state-centric serving 的路线图](../../images/state-serving/fig01_architecture_landscape.png)

*图 1. 这张图有意把“已观察到的工程响应”和“本文的前瞻性判断”分开，以避免把推演写成既成事实。*

### 3.2 不同路线，其实在回答同一个成本问题
...
这也是为什么不能简单把这些路线排成“先进”和“落后”。它们不是同一道题的不同分数，而是在不同成本结构、任务分布和基础设施成熟度下，对同一个问题给出的不同答案。
...
更直白地说，attention 不再默认是每一层都必须拥有的“普遍权利”，而开始变成一种被选择性分配的昂贵能力。
...
如果说 Hybrid 的问题意识是“哪些层可以不再背负完整历史”，那么 Sparse / MLA 的问题意识就是“哪些历史仍需要被精确寻址，但可以用更低成本保存”。两者的分歧不在于谁更懂长上下文，而在于它们对可压缩性的边界判断不同。
...
## 4. Hybrid 与 Sparse 会继续强势，但会被纳入更大的状态语法
...
### 4.1 Hybrid 真正做成的事：把多数层改造成长度无关状态
...
它让模型内部第一次出现更清晰的分工：有些部分继续承担精确召回，有些部分开始承担状态更新。这种分工本身，比具体比例更重要。

### 4.2 Sparse / MLA 真正做成的事：保住精确寻址，但压低成本
...
它不是“让模型永远记住一切”，而是让系统知道什么该放在脑内，什么该写到纸上，什么必须回到真实环境中重新验证。

### 4.4 “过渡”的含义不是淘汰，而是被重解释
...
## 5. Pure Linear 更像终局候选，但难点不只在算子

如果接受上一节的结论，下一步几乎自然会问：

> 那为什么不继续往前，把剩下的 attention 层也拿掉？

这个问题很自然，但也最容易误导。

### 5.1 Pure Linear 不是把 attention 继续删掉
...
### 5.2 RADLADS 与 HALO 证明“可以转”，不等于证明“终局很容易”
...
这也是 Pure Linear 最容易被低估的地方。它不是单纯的架构替换问题，而是训练目标、数据组织、外部记忆协议和工具验证闭环共同变化的问题。

### 5.4 Active Horizon：真正目标不是无限记忆，而是最小可用状态
...
## 6. 大模型不会只有记忆，而会需要记忆操作系统

只要接受 `active horizon` 这个概念，一个新的系统图景就会自然出现。

长时 Agent 不是靠单个巨大上下文在工作。

它更像靠一套分层记忆系统在工作。

### 6.1 四层记忆栈：能力、状态、笔记和真值不能混在一起

图 3 给出了本文使用的四层记忆栈。

![图 3：面向 state-centric serving 的四层记忆栈](../../images/state-serving/fig03_memory_stack.png)
...
这也是为什么“记忆更长”不是答案。如果一个系统不能区分能力、状态、笔记和真值，它记得越多，反而可能越难纠错。

### 6.2 “短期记忆 / 长期记忆”不够解释真实系统
...
对企业和开发者来说，这个区分还有一个更现实的含义：谁定义外部记忆的格式，谁就开始定义 Agent 的工作方式；谁控制工具真值层，谁就控制系统最后的校验权。

### 6.3 官方工程实践已经在把外部记忆制度化
...
![图 7：长时 Agent 的状态服务闭环](../../images/state-serving/fig07_state_serving_loop.png)

*图 7. 一次长时服务循环里，真正被反复操作的对象并不是纯文本历史，而是 live state、摘要、外部记忆、工具反馈和 carry-over residue。*

### 6.5 Markdown memory 不是外挂，而是脑外存
...
真正稀缺的不是单次 prompt 的技巧，而是组织状态的能力。

---

## 7. 如果评估还停留在 recall，就看不见长时服务的失败
...
### 7.1 长上下文 benchmark 仍有价值，但边界越来越明显
...
### 7.2 从 long-context eval 到 state-serving eval

![图 8：面向 state-centric serving 的评估地图](../../images/state-serving/fig08_evaluation_map.png)

*图 8. 一旦系统被当作长时服务系统来构造，它就必须被问：哪些能力在 compaction 之后还能留存？哪些错误在 tool failure 后能恢复？哪些记忆写入只是 token 浪费？*

### 7.3 本文建议新增六类指标

下面这六类指标不一定都能立刻标准化，但它们是 state-centric serving 迟早会需要的。
...
## 8. 从缓存到资产：state artifact 可能成为新的中间层

这一节是本文最前瞻、也最需要谨慎措辞的一部分。

这里的判断不是“state market 已经存在”，而是：
...
### 8.2 state artifact 的最小生命周期

![图 4：从状态复用到状态资产化的最小生命周期](../../images/state-serving/fig04_state_market.png)
...
这也是 state artifact 和 prompt template 的关键差别：前者需要兼容性、来源、权限和恢复语义；后者往往只是一段可复制文本。
...
一旦状态对象可以被序列化、迁移、审计和计费，新的权力问题就会出现：谁定义状态格式，谁控制兼容性，谁拥有可移植状态，谁承担隐私与误恢复的责任。
...
## 9. 最强的反对意见，不是方向错了，而是时间尺度可能更长

到这里，最合理的态度不是宣布未来已经被证明，而是主动列出反对意见。
...
因为 Full Attention 解决的是高保真召回问题，而 state-centric serving 解决的是长时服务组织问题。前者可能长期强势，后者也可能同时变得越来越必要。
...
复杂度不会消失，只会在显式系统设计和隐式上下文债务之间二选一。
...
更现实的说法也许是：开放状态市场未必会很快出现，但企业内部状态仓库、平台级 resume artifact 和组织级 workflow state registry 很可能先出现。
...
## 10. 结论：下一阶段的大模型竞争，会从 token 转向 state
...
第一，长上下文的竞争主轴正在从“单轮容量”转向“服务连续性”。窗口长度仍然重要，但它已经不是全部。真正重要的是模型能否在长任务中保持计划、状态、环境和成本的连续性。

第二，只要服务时长继续增加，KV cache、prefill 带宽、resume、compaction、retrieval 和 tool verification 就会越来越成为一等系统问题。它们不再只是工程优化，而是在定义模型能否成为可靠服务。

第三，一旦 memory 的重心从 token 转向 state，模型系统就会越来越像一个可外部化、可压缩、可恢复、可审计的记忆操作系统。谁能组织状态，谁就能组织长时服务；谁定义状态接口，谁就可能定义下一阶段 Agent 平台的入口。

这并不意味着 Full Attention 会很快消失，也不意味着 Hybrid、Sparse / MLA 会被迅速淘汰。恰恰相反，它们仍然可能是未来几年最务实、最强势的现实路线。

但从更长的时间尺度看，它们都会被放进一个更大的问题里重新理解：

模型到底是在保存历史，还是在维持一种能够继续工作的状态？

如果答案是后者，那么大模型的下一阶段就不只是“上下文更长的 Transformer”，而是“围绕状态连续性构造的长时服务系统”。

这不是终局答案，而是一个方向信号。

我们正在从一次性推理，走向持续性服务；从 prompt 工程，走向 memory engineering；从 token 消耗，走向 state 组织。

真正改变的，不是模型能看见多少过去，而是它能不能带着正确的状态进入下一步。
