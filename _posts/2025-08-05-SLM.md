...
layout: post
title: '原生轻量化大语言模型'
title_en: 'Native Small Language Models'
date: 2025-08-05 11:00:00 +0800
last_modified_at: 2026-04-27 00:00:00 +0800
author: pprp
categories: tech
topics: [efficient-llm, llm-systems]
excerpt: '从架构、数据、训练和部署四个层面讨论原生小语言模型，说明轻量化不只是压缩大模型，而是一套面向效率的协同设计。'
---
{::nomarkdown}
  ...
  <body class="bg-gray-50">
    ...
    <div class="content-wrapper px-8 py-12">
      ...
      <div class="bento-grid">
        ...
        <div class="bento-side">
          <div class="highlight-card">
            <h3 class="font-semibold text-lg mb-3 text-gray-900">核心信号</h3>
            <p class="text-sm text-gray-600 mb-3">
              原生轻量化模型把效率目标提前到架构设计阶段，而不是等模型训练完成后再压缩补救。
            </p>
      ...
      <section id="introduction" class="mb-16">
        <div class="pullquote">
          原生轻量化大语言模型真正改变的，不是把大模型做小，而是把“能否低成本运行”提前变成模型设计的第一约束。与通过压缩技术将大型模型&#34;瘦身&#34;得到的轻量化模型不同，原生轻量化模型并非&#34;后天改造&#34;，而是&#34;与生俱来&#34;的精简。
        </div>

        <p class="text-lg leading-relaxed mb-6">
          过去几年，大模型能力提升常常与参数规模、训练算力和云端部署绑定在一起。这条路线推动了复杂任务能力的进步，也带来了一个越来越突出的现实问题：当模型必须运行在个人电脑、移动设备或边缘节点上时，能力再强也要先过内存、延迟和能耗这三道门槛。
        </p>

        <p class="text-lg leading-relaxed mb-6">
          传统的大模型压缩技术，如知识蒸馏、剪枝、量化等，虽然能够在一定程度上减小模型体积，但通常伴随着复杂的处理流程和一定的性能折损风险。这种&#34;先大后小&#34;的思路，就像先建造一座只适合高速公路行驶的重型车辆，再想办法让它进入城市窄路；能否跑起来，取决于后期改造是否足够精细。
        </p>
      ...
      <section id="core-concepts" class="mb-16">
        <h2 class="serif text-4xl font-bold mb-8 text-gray-900">1. 核心概念：轻量化从设计阶段开始</h2>

        <div id="native-definition" class="mb-12">
          <h3 class="text-2xl font-semibold mb-6 text-gray-800">1.1 定义：不是压缩后的大模型，而是为小环境设计的模型</h3>
          ...
          <div class="bg-blue-50 border-l-4 border-blue-400 p-6 my-6">
            <h4 class="font-semibold text-lg mb-3 text-blue-900">设计理念转变：从云端中心到端侧约束</h4>
          ...
          <p class="text-lg leading-relaxed mb-6">
            原生轻量化模型则试图打破这一范式，通过精巧的架构设计，如稀疏计算、参数共享和量化感知训练等，在尽量保留任务能力的前提下，大幅降低模型的资源占用。例如，<strong>SmallThinker系列模型</strong>通过其创新的两级稀疏结构，在仅激活少量参数的情况下完成推理，实现了与更大规模模型相竞争的性能，同时显著降低了对计算资源的需求。<a href="#ref51" class="citation">[51]</a>
          </p>
        </div>

        <div id="compression-diff" class="mb-12">
          <h3 class="text-2xl font-semibold mb-6 text-gray-800">1.2 与压缩模型的根本区别：先天约束与后天改造</h3>
        ...
        <div id="training-advantages" class="mb-12">
          ...
          <p class="text-lg leading-relaxed mt-6">
            原生训练的优势在于，它把端侧设备的效率约束纳入模型形成过程，而不是部署前的补丁。例如，<strong>SmallThinker模型</strong>通过其独特的双层稀疏架构，在推理时只调用必要的专家和神经元，降低了内存占用和计算量，使其能够在普通CPU上运行。<a href="#ref167" class="citation">[167]</a>
          </p>
        </div>
      </section>
      ...
      <section id="architecture" class="mb-16">
        <h2 class="serif text-4xl font-bold mb-8 text-gray-900">2. 架构设计：效率不是压缩出来的，而是路由出来的</h2>
        ...
        <div id="moe" class="mb-12">
          ...
          <p class="text-lg leading-relaxed mb-6">
            这种双层稀疏结构，就像是在一个大型图书馆中，首先根据主题找到相关的书架（选择专家），然后只翻阅书架上最相关的几本书（激活神经元），从而减少无效计算。根据文中引用的资料，这种设计使得SmallThinker能够在总参数量达到210亿的情况下，将激活参数量控制在30亿，形成较高的计算效率。
          </p>
        </div>
        ...
        <div id="other-innovations" class="mb-12">
          ...
          <p class="text-lg leading-relaxed mb-6">
            <strong>原生1位训练</strong>是轻量化模型领域值得关注的方向，它从模型参数表示方式入手，尝试在训练阶段就获得更高的压缩率和计算效率。微软研究院的<strong>BitNet模型</strong>直接从训练开始就使用1位（或接近1位）的数值来表示权重。<a href="#ref98" class="citation">[98]</a>
      ...
      <section id="training" class="mb-16">
        ...
        <div id="bitnet-training" class="mb-12">
          ...
          <p class="text-lg leading-relaxed mb-6">
            原生1位训练是BitNet模型提出的核心创新，它重新组织了传统模型训练与量化的关系。在传统的流程中，模型以高精度（如FP32）进行训练，然后通过一个独立的量化步骤将其转换为低精度（如INT8或更低）。<a href="#ref98" class="citation">[98]</a>
          </p>

          <div class="pullquote">
            原生1位训练从训练的第一天起就使用1位精度来表示和计算模型的所有参数。这种方法的优势在于，它把低精度约束纳入训练过程，从而减少后训练量化（PTQ）可能带来的精度损失。
          </div>
      ...
      <section id="comparison" class="mb-16">
        <h2 class="serif text-4xl font-bold mb-8 text-gray-900">5. 对比压缩模型：两条路线，两种工程取舍</h2>
        ...
        <div class="mb-12">
          <h3 class="text-2xl font-semibold mb-6 text-gray-800">5.2 性能与精度：原生设计降低折损风险</h3>
          ...
          <p class="text-lg leading-relaxed mb-6">
            压缩模型则面临性能下降风险。知识蒸馏、剪枝和量化等操作，本质上都是对原始模型信息的有损压缩；如果校准、微调和任务适配处理不当，损失会累积，并拉低压缩后模型的精度。<a href="#ref30" class="citation">[30]</a>
            <a href="#ref31" class="citation">[31]</a>
          </p>
        </div>
      ...
      <section class="mb-16">
        <div class="bg-gradient-to-r from-slate-900 to-teal-900 text-white rounded-2xl p-8">
          <h2 class="serif text-3xl font-bold mb-6">技术展望：小模型的竞争会回到系统效率</h2>
          <p class="text-lg leading-relaxed mb-6">
            原生轻量化大语言模型代表着AI工程化的一条重要路线：不再把端侧部署视为大模型压缩后的附属场景，而是把算力、内存、延迟和隐私约束作为模型设计的起点。它的价值不只是“更小”，而是让模型能力更容易进入边缘计算、移动设备和物联网等真实环境。
          </p>
          <p class="text-lg leading-relaxed">
            后续竞争很可能不只发生在参数规模和基准分数上，也会发生在路由效率、缓存管理、量化友好性和硬件适配上。原生轻量化模型不是大模型路线的替代品，而是提醒我们：当AI进入日常设备，系统效率本身就是能力的一部分。
          </p>
        </div>
      </section>
...
@misc{dong2025slm,
  ...
  month = aug,
  day = {5},
  howpublished = {\url{https://pprp.github.io/tech/SLM/}},
  url = {https://pprp.github.io/tech/SLM/},
