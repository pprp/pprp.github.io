...
## 先说结论：DeepSeek-V4 不是只把窗口拉到 1M

如果只看标题，DeepSeek-V4 最容易被理解成又一个“支持 100 万 token 上下文”的模型。

但这篇论文真正值得读的地方，不是窗口长度本身，而是它把百万 token 上下文拆成了一整套协同问题：注意力怎么降复杂度，KV cache 怎么压，长前缀怎么复用，MoE 通信怎么隐藏，低精度怎么进训练和推理，post-training 又如何让模型真的会用长上下文做 agent 任务。
...
这意味着 DeepSeek-V4 的问题意识不是“我能不能看到更长历史”，而是：

> **当模型被要求持续读、持续想、持续调用工具时，能不能把长上下文的成本压到可训练、可部署、可复用的范围内？**

这个问题比单纯扩大 context window 更难。因为上下文长度一旦进入百万级，瓶颈会同时出现在计算、显存、带宽、缓存布局、分布式训练和 agent runtime 里。

## 为什么百万上下文首先是 serving 问题
...
所以 DeepSeek-V4 的主线可以画成这样：

![DeepSeek-V4 系统协同栈示意图](../../images/deepseek-v4/deepseek-v4-system-stack.svg)
...
这张图里最重要的是中间两条线：DeepSeek-V4 不是只在模型架构上动刀，也在 serving cache、kernel、通信和 post-training 上同时做配套。

## CSA 与 HCA：用两种压缩注意力分工
...
真正有意思的是，DeepSeek-V4 没有把 CSA 或 HCA 作为单独答案，而是把它们交错使用。CSA 负责较精细的长程依赖，HCA 负责便宜的全局背景，SWA 再补足最近窗口里的未压缩细节。这个组合说明论文作者并不相信“单一稀疏策略”足够覆盖所有长上下文需求。
...
论文也承认，这些稳定性机制有效，但底层原理仍没有完全解释清楚。这点反而很重要：DeepSeek-V4 的许多设计是系统工程和经验科学共同推进的结果，不是每个 trick 都已经有漂亮理论。

## KV cache：从 PagedAttention 假设走向异构状态管理
...
这些内容读起来不像“模型能力”，但它们决定了模型能力能不能以合理成本落地。

## Post-training：把长上下文变成 agent 能力
...
Flash 和 Pro 的差异也很清楚：知识任务更依赖参数规模，Pro 优势明显；数学和代码推理中，Flash 在更大 thinking budget 下可以接近 Pro；复杂 agent 任务上，Pro 仍然更稳。
...
这也是 DeepSeek-V4 对后续开源模型最重要的启发。长上下文不会只靠更大的 context window 自然变好，它需要模型和系统一起重新设计。
